...
pushd studio-se-master
  pushd talend.studio.parent.pom
    mvn clean install
  popd
  mvn clean install --log-file ../logs/build-tos.log
#  -Dproduct.timestamp=$version
#  -Dbuild.plugins.plugin.version=$version
...
#  Missing at the document: -Dtos.mdm=[true|false]
popd
